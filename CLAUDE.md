...
Fases 0 a 3 cerradas (5-sep-2026): schema, siete endpoints, admin en `/admin`, shell (ruta como estado, truncado, cookie, umbral) y DATOS completo (tabla por colección, hoja del record, columnas en perspectiva, teclado, ir a, facetas). Sigue la Fase 4 (DISEÑO: proyectos), con `/nueva-seccion`. Cambiar el schema implica migración nueva y revisar `app/lib/fieldMeta.ts`.
