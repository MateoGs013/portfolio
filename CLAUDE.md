...
Fases 0 a 3 cerradas (5-sep-2026): schema, siete endpoints, admin en `/admin`, shell (ruta como estado, truncado, cookie, umbral) y DATOS completo (una carpeta por pantalla: índice en la raíz, tabla por colección, hoja del record con vecinos, teclado de carpeta, ir a, facetas). Sigue la Fase 4 (DISEÑO: proyectos), con `/nueva-seccion`. Cambiar el schema implica migración nueva y revisar `app/lib/fieldMeta.ts`.
