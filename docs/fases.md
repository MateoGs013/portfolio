...
**Revisión de UX (5-sep-2026):** con capturas del recorrido completo se vio que la navegación directa se había perdido (nombres truncados en columnas angostas, stack escondido detrás de un tercer nivel, metadata del schema en el panel grande). La colección pasó a ser una tabla, el record una hoja plana con las relaciones a la vista, y el sub-nivel desapareció. **Segunda revisión, la misma noche:** las columnas en perspectiva y la tabla abrumaban; DATOS pasó a ser un explorador de archivos (`DatosFolder.vue` para la base y para cada colección, `DatosDetail.vue` para la hoja con vecinos), con la ruta como barra de dirección y teclado de foco itinerante. Detalle en `docs/decisiones.md`.
